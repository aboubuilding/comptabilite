 ...
 */
    ...
    public function charger($id)
    {
        ...
        $souscription_scolarite = Souscription::getPrix($niveau->id,$annee_id, $id, null, TypePaiement::FRAIS_SCOLARITE );

        $souscription_assurance = Souscription::getPrix(null,$annee_id, $id, null, TypePaiement::FRAIS_ASSURANCE );

if(!$souscription_scolarite){

    // generer souscription ecolage

    $frais_scolaire = FraisEcole::getPrix(TypePaiement::FRAIS_SCOLARITE, null, $inscription->niveau_id, $annee_id );

        Souscription::addSouscription(
            Carbon::today(),
            $frais_scolaire->montant,
            null,
        ...
        if(!$souscription_assurance){

            // generer souscription ecolage

            $frais_assurance = FraisEcole::getPrix(TypePaiement::FRAIS_ASSURANCE, null,null ,$annee_id );

            Souscription::addSouscription(
                Carbon::today(),
                $frais_assurance->montant,
                null,
                TypePaiement::FRAIS_ASSURANCE,
        ...
        if($inscription->type_inscription == TypeInscription::INSCRIPTION){
            ...
            $souscription_inscription = Souscription::getPrix(null,$annee_id, $id, null, TypePaiement::FRAIS_INSCRIPTION );
            $frais_inscription = FraisEcole::getPrix(TypePaiement::FRAIS_INSCRIPTION, null,null ,$annee_id );

            if(!$souscription_inscription){
        ...
        $frais = Souscription::getListe(null,$annee_id,  $id);

        foreach($frais as $frai ){
            $data []  = array(

                $montant_deja =  Detail::getMontantTotal($annee_id, null, null, $id, null, StatutPaiement::ENCAISSE, null, null, $frai->id ),

                $montant_prevu = (int) $frai->montant_annuel_prevu,

                    "id"=>$frai->id,
                "libelle"=> $frai->frais_ecole_id == null ? ' ' : $frai->fraisecole->libelle,
                "montant_prevu"=> $frai->montant_annuel_prevu == null ? ' ' : $frai->montant_annuel_prevu,
